...
import { View, Text, ScrollView, TouchableOpacity, ImageBackground, Dimensions, ActivityIndicator, Image, Animated, Pressable } from 'react-native';
...
export default function HomeScreen({ user, onSignOut }: HomeScreenProps) {
  ...
  const renderFeedPosts = () => {
    ...
    return feedItems.map((item, index) => {
      ...
      const userIdentifier = post.userId ? `User ${String(post.userId).slice(0, 6)}` : 'Community Member';
      const authorLabel = post.username || userIdentifier;
      const fallbackImage = index < fallbackPostImages.length ? fallbackPostImages[index] : undefined;
      ...
      return (
        ...
        >
          <View style={tw`flex-row justify-between items-start mb-2`}>
            <View>
              <Text style={tw`text-base font-semibold text-black`}>{authorLabel}</Text>
          ...
          <View style={tw`flex-row justify-between items-center mt-3`}>
            <View style={tw`flex-row items-center`}>
              ...
              <Text style={tw`text-xs text-gray-600 ml-2`}>
                {(post.commentsCount ?? post.commentCount ?? (Array.isArray(post.comments) ? post.comments.length : 0))} comments
              </Text>
            </View>
            <View style={tw`flex-row items-center`}>
              {[
                ...
                >
                  ...
                  <Text style={tw`text-xs text-gray-600 ml-1`}>{reaction.count}</Text>
                </View>
              ))}
            </View>
          </View>
      ...
      >
        ...
        <View style={{ paddingBottom: 16 }}>
          <View style={{ position: 'relative' }}>
            <ImageBackground
              source={{ uri: 'https://image.tmdb.org/t/p/original/8Vt6mWEReuy4Of61Lnj5Xj704m8.jpg' }}
              style={{ width: screenWidth, height: heroHeight }}
              resizeMode="cover"
            >
              <View style={[tw`flex-1`, { backgroundColor: 'rgba(0,0,0,0.25)' }]} />
            </ImageBackground>
            <Animated.View
              style={{
                position: 'absolute',
        ...
        {!!user && (
          <Text style={tw`text-sm text-gray-500 mt-4 px-5`}>Signed in as {user.email}</Text>
        )}
