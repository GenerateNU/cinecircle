...
export const authenticateUser = (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    console.log(`[${timestamp}] Authentication failed: Missing or invalid authorization header`);
  ...
  if (!token) {
    console.log(`[${timestamp}] Authentication failed: Token not found in header`);
    return res.status(401).json({ 
  ...
  try {
    ...
    jwt.verify(token, getKey, { algorithms: ['RS256'] }, (err, decoded: any) => {
      if (err) {
        console.log(`[${timestamp}] JWKS verification failed, trying fallback: ${err.message}`);
        ...
        try {
          ...
          console.log(`[${timestamp}] Fallback JWT verification successful for user: ${fallbackDecoded.sub}`);
          ...
          console.error(`[${timestamp}] Fallback JWT verification failed:`, fallbackError)
          return res.status(401).json({ 
        ...
        console.log(`[${timestamp}] JWKS verification successful for user: ${decoded.sub}`);
